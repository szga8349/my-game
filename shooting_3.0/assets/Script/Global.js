module.exports = {
    ...
    computeScore() {
        // 小方块最大数值每一组方块增加20%
        let newNum = parseInt(this.gameData.baseNum * (1 + 0.2 * this.gameData.level));
        /** 
         * 每隔3组方块列表增加一
         * let percent = 0.5 - 0.05 * (1 + parseInt(this.gameData.level / 3));
        */
        let percent = 0.5 - 0.05 * this.gameData.level;
        if (percent <= 0.1) percent = 0.1;
        /** 
         * 计算方块最小值和最大值
         * 最大为基础数值的150%, 最小为50%，并随等级变化
        */
        let min = parseInt(newNum * (1 - percent));
        if (min < 1) min = 1;
        let max = parseInt(newNum * (1 + 0.5));
        // console.log(parseInt(Math.random() * (max - min) + min));
        return { score: parseInt(Math.random() * (max - min) + min), maxNum: max };
    },
    // 随机红绿两种颜色
    randomColor(score, type) {
        let max = this.computeScore().maxNum;
        let rVal = type == 1 ? 240 : 240 - parseInt(240 * (score / max));
        let gVal = type == 2 ? 210 : 210 - parseInt(210 * (score / max));
