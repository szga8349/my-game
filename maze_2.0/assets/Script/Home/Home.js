...
cc.Class({
    ...
    switchExplain(e, str) {
        ...
        let spawn = null, action = null;
        if (str == 'show') {
           spawn = cc.spawn(cc.scaleTo(0.5, 1, 1), cc.rotateTo(0.5, 0));
        } else {
           spawn = cc.spawn(cc.scaleTo(0.5, 0, 0), cc.rotateTo(0.5, 180));
        }
        spawn.easing(cc.easeExponentialOut(3.0));
        this.explain.runAction(spawn);
        this.scheduleOnce(() => this.explainMove = false, 0.5);
    },
    closrBar () {
    ...
    onLoad() {
        ...
        // 检测是否第一次游戏
        let first = window.localStorage.getItem('maze');
        if (!first) {
            this.switchExplain(null, 'show');
            window.localStorage.setItem('maze', 'played');
        }
    },
