...
cc.Class({
    ...
    onCollisionEnter(other, self) {
        if (other.name.includes('wall') && Global.game.gameover === false) {
            cc.log('撞到墙');
            let wall = other.node.name.split('wall-').join('');
            Global.game.gameover = true;
            this.overMove(wall);
            cc.audioEngine.play(this.fallAudio, false);
            // cc.director.loadScene('Rank');
        } else if (other.name.includes('end') && Global.game.gameover === false) {
            cc.log('撞到终点了');
            Global.game.gameover = true;
            if (Global.gameInfo.level == Global.levels.length - 1) {
                ...
                cc.audioEngine.play(this.adoptAudio, false);
                this.scheduleOnce(() => {
    ...
    onCollisionExit(other, self) {
        if (other.name.includes('arrow') && Global.game.gameover === false) {
            // 清除走过的碰撞组件
            let road = Global.game.node.getChildByName('map').children[other.node.dataNum];
            for (let i = 0; i < road.children.length; i++) {
                road.children[i].removeComponent(cc.BoxCollider);
            }
            Global.game.move_direction = other.node.dataDirection;
            cc.audioEngine.play(this.arrowAudio, false);
            other.node.destroy();
        }
    },
    /**
